...
class App extends Component {
  ...
  handleNavCount = (habit) => {
    const nav = [...this.state.nav];

    const count = 0;
    const name = "하윙";
    this.state.nav.name = name;
    this.state.nav.count = count;
    this.setState({ nav.name : name });
    console.log(`handleNavCount ${this.state.nav}`);
  };
  ...
  render() {
    return (
      <>
        <header>
          <Nav habits={this.state.habits} onNavCount={this.handleNavCount} />
        </header>
